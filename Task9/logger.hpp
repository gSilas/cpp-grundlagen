...
public:
	...
	void write(const std::string& file, long line, const std::string& message){
		...
		else if (!m_history.empty() && !mPVector.empty()){
			for(std::unique_ptr<Policy>& p : mPVector)
			{
					p->write(m_history);
					m_history = "";
			}
	...
	void registerPolicy(std::unique_ptr<Policy> _policy){
		...
		mPVector.push_back(std::move(_policy));
...
private:
	...
	static Logger* mLogInst;

	// TODO: somehow collect the policies
	std::vector<std::unique_ptr<Policy>> mPVector;
